...
def load_model(filename):
    try:
        ...
        print(f"Error: The file {filename} was not found. Ensure the path is correct.")
    except pickle.UnpicklingError:
        print(f"Error: The file {filename} could not be unpickled. It may be corrupted.")
...
xboost_SMOTE_model = load_model('xgboost_SMOTE.pkl')
...
def make_predictions(input_df, input_dict):
    ...
    with col1:
        ...
        st.write(f"The coustomer has a {avg_probability:.2f} probability of churning.")
...
import openai  # Replace with actual API library if not using OpenAI's API
import pandas as pd
...
def explain_prediction(probability, input_dict, surname):
        ...
        Here are summary statistics for churned customers: {df[df['Exited'] == 1].describe()}
        Here are summary statistics for non-churned customers: {df[df['Exited'] == 0].describe()}
...
def generate_email(probability, input_dict, explanation, surname):
    prompt = f"""You are a manager at SG Bank. You are responsible for ensuring customers stay with the bank and are incentivized with various offers.
...
# Assuming these are the top 10 features (dummy example here for demonstration)
feature_importances = {
    "Feature": [
        "NumOfProducts", "IsActiveMember", "Age", "Geography_Germany",
        "Balance", "Geography_France", "Gender_Female", "Geography_Spain",
        "CreditScore", "EstimatedSalary"
    ],
    "Importance": [
        0.323888, 0.164146, 0.109558, 0.091373, 0.052786, 0.046463, 0.045283,
        0.036855, 0.035805, 0.032655
    ]
}

# Convert feature importances into a DataFrame for display (optional)
feature_importances_df = pd.DataFrame(feature_importances)

# Display feature importances in the Streamlit app (or console)
print("Here are the machine learning model's top 10 most important features for predicting churn:")
print(feature_importances_df)

# Display summary statistics for churned customers
print("Here are summary statistics for churned customers:")
print(df[df['Exited'] == 1].describe())
...
# Create a list of customers to display in the dropdown
...
if selected_customer_option:
    ...
    st.write("Surname:", selected_surname)
